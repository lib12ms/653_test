...
class Settings(BaseSettings):
    ...
    openai_reasoning_effort: str = Field(
        ...
        description=(
            ...
            "reasoning 모델이 아닌 model(gpt-4o 등) 호출 시에는 무시됨."
        ),
    )
    openai_max_output_tokens: int = Field(
        default=1200,
        description=(
            "Responses API max_output_tokens. gpt-5는 reasoning 토큰을 이 예산에서 "
            "함께 소모하므로(reasoning_effort=minimal이어도) 기존 gpt-4o 기준(200)보다 "
            "충분히 크게 잡아야 가시 출력이 잘림 없이 생성됨."
        ),
    )
