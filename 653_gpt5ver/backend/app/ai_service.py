...
CATEGORY_PROMPTS = {
    ...
    "외국어": (
        ...
        "  예시의 언어명은 형식 참고용일 뿐 — 실제 도서의 학습 언어에 맞춰 바꿔 쓸 것(예: 중국어 책이면 뉴스중국어)\n\n"
...
LOW_VALUE_KEYWORDS = {
    ...
    "그래픽일반",  # 카테고리 체인 말단 복붙
    "활용능력",    # 카테고리 체인 말단 복붙
    "교양인문학",  # 캐치올 override된 책의 원본 카테고리 텍스트 누출
    "정보통신",
...
CATEGORY_CANDIDATE_DENY = {
    ...
    "철학",
    "인문학",
    "교양인문학",  # 캐치올 override된 책의 원본 카테고리("인문학>교양 인문학") 텍스트 누출
    "심리학",
    "건강",
    "취미",
    "요리",
...
def _is_reasoning_model(model: str) -> bool:
    """gpt-5·o1·o3 등 reasoning 모델 계열 여부.

    reasoning 모델은 `reasoning` 파라미터를 지원하고(오히려 비-reasoning 모델에
    전달하면 API 오류), 동시에 max_output_tokens 예산을 reasoning 토큰과 가시
    출력 토큰이 함께 나눠 쓴다 — 예산이 작으면 reasoning만 소모하고 가시 텍스트가
    빈 채로 잘릴 수 있다(gpt-4o→gpt-5 전환 시 겪었던 문제의 원인).
    """
    m = (model or "").lower()
...
async def _call_learned_agent_api(
    ...
    budget = max_output_tokens or settings.openai_max_output_tokens
    kwargs: dict = {
    ...
    if _is_reasoning_model(settings.openai_model):
        # effort를 낮게 고정해 reasoning 토큰 소모를 최소화 — 그래도 예산은
        # 넉넉히 잡아야 가시 출력이 안전하게 나온다(위 budget 참고).
    ...
    # reasoning 모델이 reasoning 토큰만으로 예산을 다 쓰고 가시 텍스트를 못
    # 낸 경우(status="incomplete", reason="max_output_tokens") — 예산을 2배로
    # 늘려 한 번 더 시도한다. gpt-4o→gpt-5 첫 전환 시도가 실패했던 근본 원인이라
    # 여기서 자동 복구하지 않으면 조용히 텍스트 fallback으로 품질이 떨어진다.
    if (
        not content
        and getattr(resp, "status", None) == "incomplete"
        and getattr(resp.incomplete_details, "reason", None) == "max_output_tokens"
    ):
        logger.warning(
            "reasoning 토큰이 max_output_tokens(%d)를 전부 소진, 예산 2배로 재시도", budget
        )
        kwargs["max_output_tokens"] = budget * 2
        try:
            resp = await client.responses.create(**kwargs)
            content = (resp.output_text or "").strip()
        except Exception:
            logger.exception("OpenAI Responses API 예산 확대 재시도 실패")
            raise
