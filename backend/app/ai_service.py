...
def _system_and_user_messages(
    ...
    mode_prompt = (
        ...
        "- 금지(접속 결합): '과/와/의'로 두 개념을 이어 붙인 단어(예: 도망과피난, 삶의이면)는 출력 금지. "
        "각 개념을 별도 $a로 분리하거나 대표어 하나로 치환하십시오.\n"
        "- 동의어·유사어 중복 금지: 같은 의미의 단어를 두 개 이상 출력하지 마십시오. "
        "대표어 1개만 선택하십시오. (예: '신세대'+'젊은세대' → '청년세대' 하나만)\n"
    ...
    global_principles = (
        ...
        "책의 감상을 형용사(예: 따뜻한, 슬픈)로 표현하지 마십시오. "
        "대신 독자가 처한 사회적 상황, 정체성, 해결하고자 하는 문제(Problem-solving)를 키워드로 치환하십시오. "
