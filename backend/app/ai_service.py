...
_STATIC_INSTRUCTIONS = (
    ...
    "### 1. 독립성 (Independence): 제목·저자와 중복되지 않는 독립 주제어\n"
    "제목(245)·저자(1XX/7XX)에 이미 포함된 단어를 그대로 반복하지 마십시오. "
    "653은 제목·저자 정보를 보완하는 별도의 주제 접근점이어야 합니다. "
    "단, 관련 주제라도 구체 하위개념으로 치환하면 허용됩니다.\n\n"
    "### 2. 구체성 (Specificity): 포괄적 상위 분류명 단독 사용 금지\n"
    "'자연과학', '과학', '인문학', '역사', '에세이', '자기계발', '예술', '교육', '사회과학' 같은 "
    "포괄적 상위 분류명을 단독 키워드로 사용하지 마십시오. "
    "도서의 핵심 실체를 드러내는 구체적 하위 개념을 추출하십시오. "
    "(예: 자연과학·과학 → 양자역학 / 인문학 → 실존주의 / 역사 → 조선후기 / 자기계발 → 시간관리)\n\n"
    "### 3. 목적성 (Purpose): 검색의도를 반영한 유효한 명사·명사구\n"
    "이용자가 검색창에 실제로 입력할 법한 명사·명사구를 선택하십시오. "
    "감상 형용사(따뜻한, 슬픈, 감동적, 여운)로 표현하지 마십시오. "
    "판촉어라도 검색 효용이 있으면 허용합니다. (예: '힐링', '동기부여', 'N잡러')\n"
    "독자가 처한 사회적 상황, 정체성, 해결하고자 하는 문제를 키워드로 치환하십시오. "
    "(예: '위로' → '번아웃', '마음치유' / '성장' → '자존감', '회복탄력성')\n\n"
    "### 4. 시의성 (Timeliness): 대중이 활발히 쓰는 신조어·트렌드어 적극 허용\n"
    "도서관 통제어 체계에만 갇히지 마십시오. "
    "현재 이용자들이 해당 주제에 사용하는 신조어·트렌드어를 적극 반영하십시오. "
    "(예: '거대언어모델' 옆에 'LLM' 병기 / '부업' 옆에 'N잡러' 허용)\n\n"
    "## 653 키워드 선정 핵심 임무\n"
    "**이 책의 독자가 구글 검색창에 입력할 법한 1~2단어 구성의 검색 키워드를 추출하세요.**\n"
    "실제로 검색창에 치지 않을 단어(추상어, 감상어, 책 속 사물·사례)는 제외합니다.\n\n"
    "## 핵심 판단: 주제어 vs 내용어\n"
    "책에 '등장'하는 사물·사례·인물·현상은 내용어입니다. 653 키워드는 '책이 무엇을 다루는가'를 가리키는 주제어여야 합니다.\n"
    "내용어 → 주제어 치환 필수 예시:\n"
    ...
    "너무 구체적(사례·인스턴스 수준)이면 한 단계 위 주제로 치환하십시오.\n\n"
    "### [형식·필터 규칙]\n"
    "- 형식: 2~6글자 복합명사, 붙여쓰기(공백 없음)\n"
    "- 제외: 제목·저자 유래어 (기술서 도구명은 허용)\n"
    "- 제외: '과/와/의'로 이어 붙인 결합어 → 각 개념을 분리하거나 대표어 하나로 치환\n"
    "  (예: 도망과피난 → 도망·피난 각각 / 삶의이면 → 삶·이면 분리)\n"
    "- 제외: 동의어·유사어 중복 → 대표어 1개만. `~적` 형용사형과 명사형이 겹치면 명사형을 선택하십시오. "
    "(예: 수학적사고·수학사고 → 수학사고 / 창의적사고·창의력 → 창의력 / 신세대·젊은세대 → 청년세대)\n"
    "- 제외: 한국·국내+문학장르 결합어 (한국문학, 국내소설, 현대한국소설 등) — 외국 국가+장르(일본소설, 러시아문학 등)는 허용\n"
    "- 제외: 한정어 없는 단순 장르명 (소설, 에세이, 수필, 희곡 등; 성장소설·추리소설은 허용)\n"
    "- 제외: 추상 접미어 결합어 끝이 의미·이면·반추·가치관·문체·정서·사유·고찰·성찰·탐색·조명·인식론·존재론인 단어\n"
    "- 배경 키워드(시대·지역)는 설명·목차에 명시된 내용에서만 추출. 저자나 작품에 대한 배경지식으로 추정하여 생성 금지\n"
    "- 문학에서 '한국소설'·'국내소설'·'현대한국소설'류 → 성장소설·심리소설·도시소설 등으로 치환. 외국 국가+장르(일본소설, 러시아문학 등)는 허용\n"
